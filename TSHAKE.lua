...
if text == 'اسماعيل' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = 'سمعوو مشغول سولف شتريد 😐😕'
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'انام' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = 'نوم الهنى حبيبي مح من شفتك 😍😘😬'
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
   ...
   .."● - `انقضاء البوت` : "..exp_dat.." `يوم`\n" .."ֆ • • • • • • • • • • • • • ֆ"
	...
	-----------------------------------------------------------------------------------------------
  	if text:match("^[Dd][Ee][Vv]$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
       sendContact(msg.chat_id_, msg.id_, 0, 1, nil, 9647727411909, 'صًمـْتہ  ٳلہحہنٍٳنٍ ', '', bot_id)
    end
	-----------------------------------------------------------------------------------------------
